...
```powershell
pytest tests/ -q                     # правила экономики и фильтр безопасности
...
Первая проверяет то, что нельзя увидеть глазами: расчёт награды и штрафа,
уровень сцены, поведение защитного фильтра. Ошибка в этих местах не
проявляется падением — приложение продолжает работать, просто начисляет не
те числа.

Вторая сверяет фильтр безопасности между настольной и мобильной версиями:
он существует в двух видах, и расхождение означало бы, что на телефоне
