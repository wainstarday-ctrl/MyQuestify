...
def main() -> int:
    ...
    try:
        ...
        # private_mode=False обязателен: по умолчанию PyWebView открывает окно
        # в приватном режиме, где локальное хранилище не переживает закрытие
        # приложения. Интерфейс держит там памятку оформления — язык и тему, —
        # чтобы экран загрузки появлялся сразу верным, не дожидаясь ответа
        # сервера. С приватным режимом памятка стиралась при каждом выходе, и
        # заставка всегда показывалась по-русски, какой бы язык ни был выбран.
        #
        # Хранилище кладётся к остальным данным приложения, а не во временный
        # каталог: иначе оно точно так же исчезало бы при уборке системы.
        webview.start(
            gui=gui_backend,
            debug=not IS_FROZEN,
            private_mode=False,
