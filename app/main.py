...
async def apply_overdue_penalties(session: AsyncSession) -> tuple:
    ...
    Проверку запрашивает интерфейс: при запуске, при возвращении к свёрнутому
    окну и раз в несколько минут, пока окно открыто. Фонового планировщика на
    стороне сервера нет намеренно — мобильная версия работает без него, а два
    источника списания разошлись бы в правилах.
