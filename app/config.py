...
#
# Для выпусков источником истины служит метка в репозитории: сборка берёт
# версию из неё, а к этому значению обращается лишь при запуске вручную.
# Прежде версия читалась только отсюда, и достаточно было забыть её
# поднять, чтобы файлы выпуска v1.9.0 назывались 1.7.0.
APP_VERSION: Final[str] = "3.0.6"
...
SCENES: Final[dict] = {
    ...
    "lab": {
        ...
        "description": {"ru": "Колбы с растворами на лабораторном столе. Возьми любую и наклони — жидкость польётся. Попадёт в другую колбу — цвета смешаются.", "en": "Flasks of reagents on a lab bench. Pick one up, tilt it and pour. Hit another flask and the reagents react."},
        "price": 2000,
    },
}
