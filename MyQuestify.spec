...
if not SKIP_LLM:
    try:
        ...
        # На машине сборки отсутствие движка — это неполадка, а не выбор.
        # Выпуск «с моделью» несёт около гигабайта весов, и без движка
        # приложение молча переходит на заготовленные фразы: снаружи такой
        # архив неотличим от рабочего. Поэтому в сборочном окружении
        # выставляется MYQUESTIFY_REQUIRE_LLM=1, и отсутствие пакета
        # обрывает сборку здесь — там, где о нём становится известно.
        #
        # На своей машине переменная не выставлена, и сборка без модели
        # проходит как прежде: это законный способ получить лёгкий выпуск.
        if os.environ.get("MYQUESTIFY_REQUIRE_LLM") == "1":
            raise SystemExit(
                "[MyQuestify] llama-cpp-python не установлен, а сборка "
                "объявлена как требующая движка (MYQUESTIFY_REQUIRE_LLM=1). "
                "Выпуск с моделью без движка был бы нерабочим."
            )
