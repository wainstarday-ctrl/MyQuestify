...
# Сборщик прослеживает зависимости по импортам Python. Нативная библиотека
# llama.cpp импортом не подключается: пакет llama-cpp-python загружает её
# средствами ctypes из своего каталога, и без явного указания она в
# приложение не попадает. Для многих пакетов эту работу берут на себя
# готовые предписания сборщика, но для llama-cpp-python такого нет —
# спецификация для Windows собирает библиотеки явным вызовом, а здесь
# соответствующий ключ отсутствовал.
#
# Последствие было тихим и дорогим: приложение собиралось без движка,
# образ диска «с моделью» вёз около гигабайта весов, а приложение при
# запуске сообщало, что модель не найдена.
LLM_ARG=''
if [[ $NO_LLM -eq 0 ]] && python -c 'import llama_cpp' 2>/dev/null; then
    LLM_ARG='--hidden-import llama_cpp --collect-binaries llama_cpp --collect-data llama_cpp'
    ok 'движок инференса будет вложен в приложение'
elif [[ "${MYQUESTIFY_REQUIRE_LLM:-}" == '1' ]]; then
    # Сборочное окружение объявляет, что движок обязателен. Собрать
    # приложение без него означало бы выпустить заведомо нерабочий образ.
    fail 'llama-cpp-python не установлен, а сборка объявлена как требующая движка.'
else
    warn 'движок инференса не установлен — приложение соберётся на резервных фразах'
fi
