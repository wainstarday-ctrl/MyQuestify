 ...
 *
 * Колбы с реактивами. Колбу берут указателем и наклоняют клавишами поворота,
 * а на сенсорном устройстве — экранными кнопками в углу сцены; за порогом
...
(function (global) {
  ...
  global.Stage.register('lab', function (env) {
    ...
    return {
      usesPhysics: false,
      hint: 'Возьми колбу и наклони её. Перелей в соседнюю — реагенты вступят в реакцию.',
      ...
      render: function (ctx, width, height) {
        ...
        flasks.forEach(function (flask) {
          ...
          if (flask === dragged && Math.abs(flask.angle) < 0.06) {
            ...
            // На устройстве без клавиатуры вместо имён клавиш стоят знаки
            // экранных кнопок — тех самых, что показаны в углу сцены.
            // Подпись «Q / E» там обещала управление, которого нет.
            var tilt = global.TouchControls && global.TouchControls.isNeeded()
              ? '↺ / ↻'
              : keys.ccw.replace('Key', '') + ' / ' + keys.cw.replace('Key', '');
            ctx.fillText(
              tilt + ' — ' + env.t('lab.tilt', 'наклон'),
