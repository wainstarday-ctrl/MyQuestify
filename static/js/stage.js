 ...
 * Порядок изложения
 * -----------------
 * 1. Постоянные величины и профили отрисовки — таблица PROFILES задаёт
 *    долю частиц, предел плотности пикселей и разрешение свечения для
 *    трёх уровней устройств.
 * 2. Состояние ядра (S) и вспомогательные расчёты.
 * 3. Определение профиля: detectProfile по сведениям об устройстве,
 *    applyProfile для применения, considerDowngrade для понижения по
 *    измеренной частоте кадров.
 * 4. Размер холста и цикл кадров: applySize, frame, start, stop.
 * 5. Ввод: указатель, перетаскивание тел, экранные кнопки поворота.
 * 6. Открытый интерфейс: init, mount, status, destroy.
 *
 * Соответствие профиля и нагрузки проверяется без браузера —
 * tools/check_stage_profile.js.
 */
(function (global) {
  'use strict';
  ...
  var env = {
    ...
    /**
     * Переводит подпись, рисуемую сценой на холсте.
     *
     * Подписи внутри сцен не размечаются атрибутами и в общей подстановке
     * не участвуют: они рисуются кистью, а не размещаются в дереве
     * документа. Помощник избавляет каждую сцену от проверки наличия
     * модуля перевода.
     *
     * @param {string} key ключ словаря
     * @param {string} ru русский вариант, он же запасной
     * @returns {string}
     */
    t: function (key, ru) {
  ...
  var Stage = {
    ...
    mount: function (key) {
      ...
      try {
        ...
        if (S.hint) {
          // Перевод берётся по ключу сцены, русский текст из самой сцены
          // остаётся запасным значением. Хранить подсказки в сценах и
          // переводить их в одном месте удобнее, чем разносить один и тот
          // же текст по двум файлам.
          S.hint.textContent = global.I18n
            ? global.I18n.t('hint.' + key, S.scene.hint || '')
            : (S.scene.hint || '');
        }
    ...
    /**
     * Обновляет текст подсказки под текущий язык.
     *
     * Вызывается при смене языка. Пересборка сцены ради подписи была бы
     * расточительной: она сбросила бы положение предметов и запас топлива,
     * то есть состояние, накопленное пользователем.
     */
    retranslateHint: function () {
      if (!S.hint || !S.scene || !S.sceneKey) { return; }
      S.hint.textContent = global.I18n
        ? global.I18n.t('hint.' + S.sceneKey, S.scene.hint || '')
        : (S.scene.hint || '');
    },
