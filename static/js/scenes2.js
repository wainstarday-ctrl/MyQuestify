    ...
    return {
      ...
      update: function (dt) {
        ...
        leaves.forEach(function (leaf) {
          ...
          if (depth > 0) {
            // Архимед: выталкивающая сила равна весу вытесненной воды.
            //
            // Расчёт ведётся от собственного веса тела. Прежний вариант
            // умножал плотность воды на глубину и радиус напрямую, минуя
            // множитель силы тяжести движка, равный одной тысячной. Сила
            // выходила примерно в тысячу раз больше веса, и лист мгновенно
            // прижимался к поверхности, не успевая упасть.
            //
            // Единицы измерения в движке условны, поэтому величины
            // надёжнее сопоставлять между собой, а не подбирать по
            // отдельности: отношение подъёмной силы к весу проверяемо,
            // абсолютное значение — нет.
            var submergedPart = Math.min(1, depth / (leaf.circleRadius * 2));

            // Множитель силы тяжести в движке: вес тела за шаг равен
            // массе, умноженной на это значение.
            var weight = leaf.mass * 0.001;

            // Больше единицы — тело легче воды и всплывает. Подобрано так,
            // чтобы лист выходил на поверхность за секунду и покачивался,
            // а не выпрыгивал.
            var lift = weight * submergedPart * 1.9;
