...
(function (global) {
   ...
   *
   * Вычисляется при загрузке модуля, а не в init(): это свойство устройства,
   * а не состояние работы. Прежде признак выставлялся только внутри init(),
   * который вызывается из ядра сцен — то есть позже настройки интерфейса. В
   * тот момент isNeeded() отвечал «нет» на любом устройстве, и настройка
   * клавиш поворота оставалась видимой на телефоне, обещая управление,
   * которого там нет.
   */
  var enabled = isTouchDevice();
