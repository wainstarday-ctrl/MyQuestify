...
(function () {
  ...
  /**
   * Согласует переключатель напоминаний с разрешением системы.
   *
   * Настройка включена по умолчанию, а разрешение на устройстве по умолчанию
   * не выдано. Из-за этого переключатель показывал включённые напоминания,
   * которых на самом деле не было: разрешение запрашивалось только при
   * нажатии, а нажимать на уже включённое никто не станет.
   *
   * Теперь положение переключателя означает «напоминания работают», а не
   * «напоминания разрешены в настройках приложения»: без разрешения системы
   * он показывает выключенное состояние, и нажатие запрашивает разрешение —
   * в том самом месте, где намерение пользователя очевидно.
   *
  ...
  function initReminders() {
    ...
    return window.MobileNotify.init().then(function (state_) {
      if (!state_.shell) { return; }

      var wanted = state.settings.notifications_enabled !== false;
      dom.notifyToggle.checked = wanted && state_.granted;

      // Расписание передаётся системе заранее и само не обновляется. Пока
      // этого вызова не было, напоминания назначались только при изменении
      // списка квестов — то есть у того, кто ничего не менял, не приходили
      // никогда.
      if (wanted && state_.granted) { resyncReminders(); }
    });
  }
  ...
  function startSession() {
    ...
    api.getSettings()
      ...
      // Согласование идёт после настроек и вне зависимости от их загрузки:
      // напоминания должны быть назначены даже когда настройки не пришли и
      // остались значения по умолчанию.
      .then(initReminders);
