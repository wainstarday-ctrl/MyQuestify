    ...
    function build(width, height) {
      ...
      if (planets.length !== count) {
        ...
        for (var p = 0; p < count; p += 1) {
          ...
          planets.push({
            orbit: p,
            // Постоянный номер планеты в SOLAR. Отделён от номера орбиты:
            // орбита меняется при обмене местами, а принадлежность имени и
            // прочих неизменных свойств — нет.
            id: p,
            angle: u.rand(0, TAU),
      ...
      render: function (ctx, width, height) {
        ...
        ordered.forEach(function (planet) {
          ...
          if (planet.name) {
            ...
            // Ключ строится из постоянного номера планеты, а не из её
            // русского названия: перевод не должен зависеть от написания.
            // И не из номера орбиты: тот меняется при обмене местами, из-за
            // чего в английской версии планета получала чужое имя, тогда
            // как в русской имя следовало за планетой (там ключей нет и
            // берётся запасное значение planet.name).
            ctx.fillText(
              env.t('planet.' + planet.id, planet.name),
